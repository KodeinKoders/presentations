    ...
    SubSlide(state > 0, fontGrow) {
        Flex(FlexDirection.Row, attrs = {
            ...
            Flex(
                ...
                P({ shownIf(state > 0, Transitions.fontGrow) }) { Text("Data structure") }
                P({ shownIf(state > 1, Transitions.fontGrow) }) { Text("Business logic") }
                P({ shownIf(state > 2, Transitions.fontGrow) }) { Text("UI Behavior") }
                P({ shownIf(state > 3, Transitions.fontGrow) }) { Text("Tests!!!") }
            }
            Div({
                css {
